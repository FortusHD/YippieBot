		...
		if (interaction.user.id === config.get('ADMIN_USER_ID')) {
			...
			if (wichtelChannel) {
						...
						for (let i = 0; i < matches.length; i++) {
							...
							interaction.client.users.fetch(match[0].id).then(user => {
								const matchEmbed = new EmbedBuilder()
									...
									(Steam: ${match[1].steamName})\nÜberlege dir ein schönes Spiel für deinen Partner 
									und kaufe es auf Steam und lege es als Geschenk für den ${data.wichtelTime} 
									oder früher fest.`);
				...
				await editInteractionReply(interaction, {
					content: 'Das Wichteln wurde beendet!',
					ephemeral: true
				});
			} else {
